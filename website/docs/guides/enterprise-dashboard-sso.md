...
Hermes separately signs the provider-ownership proof carried beside each browser or Desktop refresh token. Its keyring is created atomically at `$HERMES_HOME/secrets/dashboard_refresh_binding_keys.json`, is owner-only (`0600`), and belongs in the same protected, profile-scoped persistent volume and encrypted backup policy as the rest of the deployment's secret state. Do not copy this file between unrelated profiles or expose it through configuration, command arguments, logs, browser artifacts, or support evidence. A missing keyring is initialized on first use; corrupt, broadly readable, or structurally mismatched state is never overwritten automatically and causes refresh ownership verification to fail closed until an operator restores a known-good protected copy or requires fresh sign-in.

Run `hermes dashboard sso rotate-refresh-binding-key`, then restart the dashboard, to activate a new ownership key. Hermes mints new proofs with that key and retains exactly one previous generation for existing sessions. A second rotation retires the oldest generation, so allow at least the maximum refresh-session lifetime between rotations unless intentionally forcing those sessions to sign in again. Back up the rotated keyring only through the protected secret-state workflow; never paste its contents into a ticket or deployment command.
