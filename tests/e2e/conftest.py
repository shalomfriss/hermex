...
async def send_and_capture(adapter, text: str, platform: Platform, **event_kwargs) -> AsyncMock:
    ...
    Await the processing task rather than polling a wall-clock deadline:
    handler DB work hops to worker threads (AsyncSessionDB), so completion
    latency varies substantially on shared CI runners.
    """
