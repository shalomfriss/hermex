...
Caddy supplies the fixed external `X-Forwarded-Host`, `X-Forwarded-Proto: https`, and `X-Forwarded-Port: 443`. Both Caddy and Hermes bind only `127.0.0.1`; Hermes starts with `--require-auth`, which keeps the configured SSO gate authoritative on a loopback bind intended for a trusted local reverse proxy. Hermes must configure only loopback addresses under `dashboard.trusted_proxies`. Caddy strips upstream `Server` headers and does not terminate public TLS; ngrok owns the public certificate and stable endpoint. Caddy's reverse proxy preserves WebSocket upgrades and disables response buffering.
